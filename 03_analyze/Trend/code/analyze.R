...
#Trend  model
mod_Trend_Nintendo <- SSModel(data$Nintendo[1:30] ~ SSMtrend(2, Q = c(list(0), list(NA))), H = NA)
fit_Trend_Nintendo <- fitSSM(mod_Trend_Nintendo, numeric(2), method = "BFGS")
ksf_Trend_Nintendo <- KFS(fit_Trend_Nintendo$model)

mod_Trend_Tokyo <- SSModel(data$Tokyo_denryoku[1:30] ~ SSMtrend(2, Q = c(list(0), list(NA))), H = NA)
fit_Trend_Tokyo <- fitSSM(mod_Trend_Tokyo, numeric(2), method = "BFGS")
ksf_Trend_Tokyo <- KFS(fit_Trend_Tokyo$model)

mod_Trend_Topix <- SSModel(data$Topix[1:30] ~ SSMtrend(2, Q = c(list(0), list(NA))), H = NA)
...
mod_LocalTrend_Tokyo <- SSModel(data$Tokyo_denryoku[1:30] ~ SSMtrend(2, Q = c(list(NA), list(NA))), H = NA)
...
mod_LocalTrend_Topix <- SSModel(data$Topix[1:30] ~ SSMtrend(2, Q = c(list(NA), list(NA))), H = NA)
